...
from algorithms.cross_evaluate import cross_evaluate # legacy

def _adaboot(x_train, y_train, x_test=None, y_test=None):
    clf = AdaBoostClassifier(RandomForestClassifier(min_samples_split=10, n_estimators=500))
...
def _cross_evaluate():
    ...
    cross_evaluate(_adaboot, combined_features, labels, split=4)
...
def _split_evaluate():
    ...
    clf = adaboot_split(combined_features, labels)
...
def _run_for_test(output_path):
    ...
    clf = _adaboot(train_features, train_labels)
...
if __name__ == '__main__':
    # adaboot('../output/knn/knn.txt')
    # _cross_evaluate()
    _split_evaluate()
    _run_for_test('../output/test/prediction_oe.csv')