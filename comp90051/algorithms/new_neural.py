...
def _perceptron(x_train, y_train, x_test=None, y_test=None):
    clf = MLPClassifier(activation='tanh', solver='lbfgs', alpha=1)
...
def perceptron_split(x, y):
    ...
    # t11 = read_from_txt('../output/adar/adar_test_huge.txt')['score']
...
def _cross_evaluate():
    ...
    cross_evaluate(_perceptron, combined_features, labels, split=4)
...
def _split_evaluate():
    ...
    combined_features, labels = _read_data(data_path)
    print(np.corrcoef(combined_features.T))

    clf = perceptron_split(combined_features, labels)
...
def _run_for_test(output_path):
    ...
    clf = _perceptron(train_features, train_labels)
...
if __name__ == '__main__':
    # perceptron('../output/knn/knn.txt')
    # _cross_evaluate()
    _split_evaluate()
    _run_for_test('../output/test/prediction_oc.csv')